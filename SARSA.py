    ...
    def SARSA(self):
        for episode in range(self.total_episodes):
            ...
            for step in range(self.max_steps):
                ...
                self.new_state, self.reward, self.done, self.info, self.tail_gate, self.head = list[0], \
                                                                                               list[1], \
                                                                                               list[2], \
                                                                                               list[3], \
                                                                                               list[4], \
                ...
                for (x, y), value in np.ndenumerate(self.new_state):
                    if self.new_state[x, y] == 255.0:
                        self.apple_new_position = (x, y)

                wall_info = tuple(self.is_wall_near_by(self.head))
                self.cur_new_state = self.tail_gate[0] + self.apple_new_position + self.tail_gate[-1] + wall_info

                if self.cur_new_state not in self.q_table:
                    self.flag = 1
                    self.q_table[self.cur_new_state] = {val: 0 for val in self.valid_dir}

                self.env.render()
                if numpy.random.random() > self.epsilon:
                    self.action_ = max(self.q_table[self.cur_new_state].items(), key=operator.itemgetter(1))[0]
                else:
                    self.action_ = self.env.action_space.sample()
                self.next_value = self.q_table[self.cur_new_state][self.action_]
                self.q_table[self.cur_state][self.action] += self.learning_rate * (
                            self.reward + self.gamma * self.next_value - self.q_table[self.cur_state][self.action])
            ...
            #epsilon decrement using decay rate
            self.epsilon = self.min_epsilon + (self.max_epsilon - self.min_epsilon) * np.exp(-self.decay_rate * self.episode)
            output = open('q_table_SARSA_50000_v3_final.pkl', 'wb')
            pickle.dump(self.q_table, self.output)

            output.close()
            output1 = open('q_table_SARSA_50000_v4_final_reward.pkl', 'wb')
